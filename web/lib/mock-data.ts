...
// —— /v1/environment —— 本机环境扫描的演示数据（2 台机器）
export const MOCK_ENVIRONMENT: EnvironmentResponse = {
  machines: [
    {
      machine: 'DESKTOP-WORKBENCH',
      scanned_at: minus(0.02),
      total_mcp: 7,
      total_plugin: 3,
      total_skill: 6,
      agent_count: 3,
      payload: {
        machine: 'DESKTOP-WORKBENCH',
        scanned_at: minus(0.02),
        cwd: '/home/sheep/code/aiforge',
        totals: { mcp: 7, plugin: 3, skill: 6 },
        agents: [
          {
            agent: 'claude-code',
            display: 'Claude Code',
            ...
            plugins: [
              ...
              },
              {
                ...
                scope: 'user',
                version: '0.2.0',
                path: '~/.claude/plugins/aiforge',
              },
            ],
            skills: [
              { name: 'find-skills', path: '~/.claude/skills/find-skills' },
              { name: 'verify', path: '~/.claude/skills/verify' },
              { name: 'simplify', path: '~/.claude/skills/simplify' },
              { name: 'claude-api', path: '~/.claude/skills/claude-api' },
            ],
            counts: { mcp: 3, plugin: 2, skill: 4 },
          },
          {
            agent: 'codex',
            display: 'Codex',
            ...
            plugins: [],
            skills: [
              { name: 'pdf-fill', path: '~/.codex/skills/pdf-fill' },
              { name: 'sql-explain', path: '~/.codex/skills/sql-explain' },
            ],
            counts: { mcp: 2, plugin: 0, skill: 2 },
          },
          {
            agent: 'cursor',
            display: 'Cursor',
            ...
            plugins: [
              {
                ...
                marketplace: 'cursor-extensions',
                scope: 'workspace',
                version: '0.14.3',
                path: '~/.cursor/extensions/tailwind-intellisense',
              },
            ],
            skills: [],
            counts: { mcp: 2, plugin: 1, skill: 0 },
          },
          {
            agent: 'gemini-cli',
            display: 'Gemini CLI',
    ...
    },
    {
      ...
      payload: {
        machine: 'MBP-LAPTOP',
        scanned_at: minus(1.4),
        cwd: '/Users/sheep/work',
        totals: { mcp: 2, plugin: 1, skill: 2 },
        agents: [
          {
            agent: 'claude-code',
            display: 'Claude Code',
            ...
            skills: [
              { name: 'find-skills', path: '~/.claude/skills/find-skills' },
              { name: 'remotion-best-practices', path: '~/.claude/skills/remotion-best-practices' },
            ],
            counts: { mcp: 1, plugin: 1, skill: 2 },
          },
          {
            agent: 'vscode',
            display: 'VS Code',
            ...
            mcps: [
              {
                ...
                url: null,
                env_keys: [],
                source: '~/Library/Application Support/Code/User/mcp.json',
              },
            ],
            plugins: [],
            skills: [],
            counts: { mcp: 1, plugin: 0, skill: 0 },
          },
          {
            agent: 'windsurf',
            display: 'Windsurf',
...
// 扁平去重名单（跨机器 + 跨 agent），用于交叉引用
export const MOCK_INSTALLED_NAMES: InstalledNames = {
  mcp: ['context7', 'playwright', 'sentry', 'filesystem', 'github', 'postgres', 'memory'],
  plugin: ['frontend-design', 'aiforge', 'tailwind-intellisense'],
  skill: [
    'find-skills',
    'verify',
    'simplify',
